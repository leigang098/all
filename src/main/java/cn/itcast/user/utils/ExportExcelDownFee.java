...
public class ExportExcelDownFee {
     ...
     * 导出数据 
     * */  
    public void export(XSSFWorkbook workbook,OutputStream out) throws Exception{  
        try{  
//        	for (int k = 0; k < 3; k++) {
        		XSSFSheet sheet = workbook.createSheet(sheettitle);                  // 创建工作表  
...
//                DateCellRowName.setCellStyle(columnTopStyle);   
                ...
                for(int i=0;i<dataList.size();i++){  
                    ...
                    XSSFRow row = sheet.createRow(i+1);//创建所需的行数（从第二行开始写数据）  
